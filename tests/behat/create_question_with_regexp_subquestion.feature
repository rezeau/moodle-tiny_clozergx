...
Feature: Test the cloze question editor string compilation after creating the question in the dialogue (using a regexp sub-question).
  ...
  Scenario: Create a REGEXP sub-question with 1 incorrect answer
    ...
    Then I should see "<p>{2:REGEXP:=blue, white and red#Those are the right colours~--.*blue.*#\n  Missing blue!}: are the colours of the French flag</p>" source code for the "Question text" TinyMCE editor
  ...
  Scenario: Create a REGEXP sub-question with an error
    ...
    When I click on "Insert question" "button"
    And I click on the "View > Source code" menu item for the "Question text" TinyMCE editor
    Then I should see "<p>{2:REGEXP:--.*blue.*# Missing blue!~=blue, white and red#Those are the right\n  colours}: are the colours of the French flag</p>" source code for the "Question text" TinyMCE editor
